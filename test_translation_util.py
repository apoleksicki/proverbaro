...
from translation_util import (
    _find_definition, _find_full_definition)
...
def test_convert_saluti_definition_1():
    ...
    saluti1def = _parse_single_definition(
        _read_json('saluti_def_1.json'))
    assert expected == saluti1def


def test_find_definition_of_saluti():
    expected = _read_json('converted_saluti.json')
    salutidef = _find_full_definition(
        'saluti', _read_saluti_json('saluti.json'))
